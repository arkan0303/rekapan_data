...
<?php 
    ...
    function buildSqlQuery($sql) {
        ...
        if (!empty($searchNama)) {
            $sql .= " LOWER(nama_pembeli) LIKE '%$searchNama%'";
        }
        ...
        if (!empty($searchTanggalDari) && !empty($searchTanggalSampai)) {
            ...
            $sql .= " tanggal_order BETWEEN '$searchTanggalDari' AND '$searchTanggalSampai'";
        } elseif (!empty($searchTanggalDari)) {
            if (!empty($searchNama)) {
                $sql .= " AND";
            }
            $sql .= " tanggal_order >= '$searchTanggalDari'";
        } elseif (!empty($searchTanggalSampai)) {
            if (!empty($searchNama)) {
                $sql .= " AND";
            }
            $sql .= " tanggal_order <= '$searchTanggalSampai'";
        }
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="nama" id="nama" placeholder="Cari Nama Pembeli" value="<?= $_GET['nama'] ?? ''; ?>">
                                </div>
                            </div>
                            <div class="col">
                                <div class="mb-3">
                    ...
                    <table class="table table-hover table-light" id="table_penjualan">
                        ...
                        <tbody>
                            <?php
                                ...
                                $sql .= " ORDER BY tanggal_order DESC LIMIT $limit OFFSET $offset";
                            ...
                            <?php else : ?>
                                ...
                                <?php while ($row = mysqli_fetch_assoc($query)) : ?>
                                    <tr>
                                        ...
                                        <td class="text-center align-middle" data-column="tanggal_order"><?= date("d/m/Y", strtotime($row['tanggal_order'])); ?></td>
                                        <td class="text-center align-middle" data-column="nama_pembeli"><?= $row['nama_pembeli']; ?></td>
                                        <td class="text-center align-middle money_format" data-column="total_harga"><?= $row['total_harga']; ?></td>
                                        <td class="text-center align-middle"><a href="/detail-penjualan.php?id=<?= $row['id']; ?>" class="btn btn-outline-dark btn-sm">Detail</a></td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php endif; ?>
                        </tbody>
